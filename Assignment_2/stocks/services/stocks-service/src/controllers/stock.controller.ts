...
import {sendDuplicateSymbolError, sendMalformedError, sendNotFoundError} from "../utils/errors.utils";
import {
    createStock,
    deleteStock,
    findStockByQuery,
...
export const createStockController = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    try {
        ...
        console.error(error);
        if (error instanceof Error && error.message === 'Stock with this symbol already exists') {
            sendDuplicateSymbolError(res);
            return;
        }
        next(error);
    }
...
export const getStocksController = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    try {
        ...
        const stocks = Object.keys(query).length === 0 ?
            await getStocks() :
            await findStockByQuery(query);
...
export const updateStockController = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    try {
        if (!isStockHaveAllFields(req.body) || !isStockDataValid(req.body)) {
            sendMalformedError(res);
            return;
        }
